...
function App() {
  ...
  const shouldSendRoomCode = useRef<boolean>(true)
  ...
  function setupWebsocket(){
    if (!socketRef.current || !(socketRef.current instanceof WebSocket) || socketRef.current.readyState === WebSocket.CLOSED) {
      socketRef.current = new WebSocket(buildWsConnUrl())
        ...
        if (typeof event.data === "string") {
          ...
          switch (msgTypeFromString(jsonObject["messageType"])) {
            ...
            case MsgType.ROOM_NOT_FOUND_TO_CLIENT:
              const dtoRoomNotFound: RoomNotFoundToClientDTO = jsonObject
              roomRef.current?.showRoomNotFoundHandle(dtoRoomNotFound.roomCode)
              break  
  ...
  function buildWsConnUrl(): string {
    let gameServerWssUrl = process.env.REACT_APP_GAME_SERVER_WS_URL as string
    const roomCode = searchParams.get('roomCode')
    return roomCode ? `${gameServerWssUrl}?roomCode=${roomCode}` : gameServerWssUrl
  }
  ...
  return (
      <div className="App">
        <Helmet>
          <script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-2164777390188769"
